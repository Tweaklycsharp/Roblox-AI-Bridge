...
function buildOpenAIRequestError(statusCode, rawBody, requestId, clientRequestId) {
  ...
  if (statusCode === 429 && apiCode === "insufficient_quota") {
    friendlyHint =
      "This API key has no available quota, or the project/organization spending limit has been reached. " +
      "Add billing/credits in the OpenAI dashboard, increase the spending limit if needed, " +
      "or use another API key/project with available quota.";
  } else if (statusCode === 429) {
    friendlyHint =
      "The request has been rate limited. Slow down, wait a moment, or check the project's RPM/TPM limits.";
